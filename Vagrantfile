...
Vagrant.configure(VAGRANT_API_VERSION) do |config|
  ...
  if ENV['ANSIBLE_HEKA_VAGRANT_BOXNAME']
    config.vm.box = ENV['ANSIBLE_HEKA_VAGRANT_BOXNAME']
  else
  ...
  config.vm.define :ansiblehekatest do |d|
    ...
    d.vm.provision :ansible do |ansible|
      ...
      ansible.tags = ENV['ANSIBLE_HEKA_VAGRANT_ANSIBLE_TAGS']
      ansible.skip_tags = ENV['ANSIBLE_HEKA_VAGRANT_ANSIBLE_SKIP_TAGS']
      ansible.verbose = ENV['ANSIBLE_HEKA_VAGRANT_ANSIBLE_VERBOSE']
      if ENV['ANSIBLE_HEKA_VAGRANT_ANSIBLE_CHECKMODE'] == '1'
